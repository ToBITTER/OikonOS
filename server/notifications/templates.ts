...
export function renderEmail(event: NotificationEvent, p: any) {
  ...
  switch (event) {
    ...
    case "member.invited":
      ...
      symbol = "+";
      action = p.url;
      actionLabel = "Sign in to OikonOS";
      details = `<table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="margin-top:20px;border-top:1px solid #E4EAF5;border-bottom:1px solid #E4EAF5">${detailRow("Business", businessName)}${detailRow("Your role", p.role || "Staff")}${detailRow("Login email", p.loginEmail || "Use this email address")}${detailRow("Temporary password", p.temporaryPassword || "Already supplied by your owner")}</table><div style="margin-top:18px;padding:14px 16px;background:#FFF8E0;border-left:3px solid #F9C321;border-radius:8px;color:#5E4B0B;font-size:13px;line-height:20px"><strong>Action required:</strong> Sign in with the temporary password. OikonOS will immediately ask you to create a private password before opening the workspace.</div>`;
      break;
